...
class TosVis(object):
    ...
    ####################
    def __init__(self, maxTime, autoBoot=True, showDebug=True, file=''):
        '''
        The class constructor.
    ...
    def setupRadio(self):
        '''
        [BALDO] Adaptation to work with DrawNetwork
        '''
        radio = self.tossim.radio()
        file = open(self.file,'r')
        connections = json.load(file)['connections']
        for conn in connections:
            radio.add(conn['source'],conn['target'],conn['attenuation'])
